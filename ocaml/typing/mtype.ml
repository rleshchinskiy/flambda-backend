...
let rec expand_paths_lazy paths env =
  ...
  | MtyL_strengthen (mty,p,a) ->
      ...
      MtyL_strengthen (mty,p,a)
  | MtyL_ident _ | MtyL_alias _ as mty ->
      mty
...
and expand_paths_lazy_sig_items paths env =
  ...
  function
    [] -> []
  | SigL_module (id,pres,md,rs,vis) :: rem  ->
      let md = { md with mdl_type = expand_paths_lazy paths env md.mdl_type }
      in
      let env =
  ...
  | SigL_modtype (id,mtd,vis) :: rem ->
      let mt = Option.map (expand_paths_lazy paths env) mtd.mtdl_type in
      let mtd = { mtd with mtdl_type = mt } in
...
let find_type_of_module ~strengthen ~aliasable env path =
  if strengthen then
    ...
    let mty = strengthen_lazy ~aliasable md.mdl_type path in
    Subst.Lazy.force_modtype mty
  else
